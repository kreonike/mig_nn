...
# ==============================================================================
# 4. РАБОТА С ПАЦИЕНТАМИ (КЛИЕНТАМИ) И ИХ ПОИСКОМ
# ==============================================================================
...
def search_clients_for_completer(
  ...
  try:
    ...
    q_no_spaces = re.sub(r"\s+", "", raw_q)

    # 1. РАЗБОР ШАБЛОНА: ИНИЦИАЛЫ + ТОЧНАЯ ДАТА (ДД, ДДММ, ДДММГГГГ)
    match_inits_date = re.match(
        r"^([a-zA-яёЁа-яА-Я]{1,3})(\d{1,8})$", q_no_spaces
    )
    if match_inits_date:
      inits = match_inits_date.group(1)
      digits = match_inits_date.group(2)
      ...
      if len(digits) in (1, 2):
        d_mask = f"{digits.zfill(2)}.%.%"
      elif len(digits) in (3, 4):
        day = digits[:2]
        month = digits[2:]
        d_mask = f"{day}.{month.zfill(2)}.%"
      else:
        day = digits[:2]
        month = digits[2:4]
        year = digits[4:]
        d_mask = f"{day}.{month}.{year}%"
    ...
    # 2. РАЗБОР ШАБЛОНА: ТОЛЬКО ИНИЦИАЛЫ ФИО (например: с, сд, сдг)
    match_inits = re.match(r"^([a-zA-яёЁа-яА-Я]{2,3})$", q_no_spaces)
    if match_inits:
      ...
      if len(inits) >= 3:
        o_p = f"{inits[2]}%"
        sql_o = (
            'AND ("Отчество" LIKE ? OR "Отчество" LIKE ? OR "Отчество" IS NULL'
            " OR \"Отчество\" = '')"
        )
        params = (
            f_p.lower(),
    ...
    # 3. ОБЩИЙ СКВОЗНОЙ ПОИСК (ФИО, СКЛЕЕННЫЙ ПАСПОРТ, СНОСКИ И ПОИСК ПО СПРАВКАМ)
    prefix_low = f"{clean_q}%"
    prefix_cap = f"{raw_q.capitalize()}%"
    anywhere = f"%{raw_q}%"
    digits_anywhere = f"%{q_no_spaces}%"
...
def search_clients(query: str, limit: int = 200) -> List[Dict[str, Any]]:
  """Основная функция поиска для главного окна (использует тот же единый точный алгоритм)."""
