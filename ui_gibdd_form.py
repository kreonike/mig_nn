...
class GibddFormDialog(QDialog):
    ...
    def init_ui(self):
        ...
        form_deal.addRow("Серия / № Справки:", spravka_layout)
        form_deal.addRow("Срок (лет):", self.field_srokom)
    ...
    def fill_client_data(self, c: dict):
        ...
        deal = database.get_latest_deal_info(c["id"], "Сделки")
        if deal and deal.get("НомДоговора"):
    ...
    def save_and_print(self):
        ...
        deal_data = {
            ...
            "Выдана_справкаСер": self.field_spr_ser.text().strip(),
            "ВыданаСправка№": self.field_spr_num.text().strip(),
            "Сроком": self.field_srokom.text().strip(),
            "КатегорияТС": self.field_cat.text().strip(),
            "Примечание": self.field_primech.toPlainText().strip(),
            "Психиатр": self.chk_psih.isChecked(),
            "Нарколог": self.chk_nark.isChecked(),
        }
        database.save_gibdd_deal(deal_data)
